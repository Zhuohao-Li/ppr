...
import re
from search_utils.llm_agent.generation import LLMGenerationManager, GenerationConfig
...
def compute_data_metrics(batch, use_critic=True):
    ...
    # Separate PRM and ORM scores if response_id_positions_per_turn is available
    prm_scores = []
    orm_scores = []
    prm_rewards = []
    orm_rewards = []
    
    if 'turn_mask' in batch.batch:
        token_level_scores = batch.batch['token_level_scores']  # (batch_size, seq_len)
        token_level_rewards = batch.batch['token_level_rewards']  # (batch_size, seq_len)
        turn_mask = batch.batch['turn_mask']  # (batch_size, seq_len), 1 at turn boundaries

        for i in range(turn_mask.size(0)):
            # PRM positions: all 1s except the last one
            boundary_indices = torch.nonzero(turn_mask[i]).flatten().tolist()
            if len(boundary_indices) == 0:
                continue
            if len(boundary_indices) > 1:
                prm_positions = boundary_indices[:-1]
                prm_score_sum = sum(token_level_scores[i, pos].item() for pos in prm_positions)
                prm_reward_sum = sum(token_level_rewards[i, pos].item() for pos in prm_positions)
                prm_scores.append(prm_score_sum / len(prm_positions))
                prm_rewards.append(prm_reward_sum / len(prm_positions))
            orm_pos = boundary_indices[-1]
            orm_scores.append(token_level_scores[i, orm_pos].item())
            orm_rewards.append(token_level_rewards[i, orm_pos].item())
    ...
    metrics = {
        ...
        # PRM scores and rewards (if available)
        **({
            'critic/prm_scores/mean': np.mean(prm_scores),
            'critic/prm_scores/max': np.max(prm_scores),
            'critic/prm_scores/min': np.min(prm_scores),
            'critic/prm_rewards/mean': np.mean(prm_rewards),
            'critic/prm_rewards/max': np.max(prm_rewards),
            'critic/prm_rewards/min': np.min(prm_rewards),
        } if prm_scores else {}),
        # ORM scores and rewards (if available)
        **({
            'critic/orm_scores/mean': np.mean(orm_scores),
            'critic/orm_scores/max': np.max(orm_scores),
            'critic/orm_scores/min': np.min(orm_scores),
            'critic/orm_rewards/mean': np.mean(orm_rewards),
            'critic/orm_rewards/max': np.max(orm_rewards),
            'critic/orm_rewards/min': np.min(orm_rewards),
        } if orm_scores else {}),
        # adv
    ...
    # Collect process reward metrics
    if hasattr(batch, 'process_reward_metrics') and 'process_reward_valid_rate' in batch.process_reward_metrics:
        metrics.update({
            'reward/process_reward_valid_rate': batch.process_reward_metrics['process_reward_valid_rate'],
            'reward/process_reward_valid_count': batch.process_reward_metrics['process_reward_valid_count'],
            'reward/process_reward_total_count': batch.process_reward_metrics['process_reward_total_count'],
        })
